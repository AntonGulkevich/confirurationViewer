...
void FailManager::PrepareFails()
{
	...
	{
		if (_config->GetArincIn(i) != nullptr)
		{
			s.third = i+1;
			s.fourth = sfrEmpty;
			IOSTATE state;
	...
	{
		if (_config->GetArincOut(i) != nullptr)
		{
			s.third = i+1;
			s.fourth = sfrEmpty;
			IOSTATE state;
	...
	{
		if (_config->GetCommandOut(i) != nullptr)
		{
			s.third = 0;
			s.fourth = sfrEmpty;
			IOSTATE state;
			state.dword = _config->GetCommandOut(i)->GetRegister(REGISTER_ID::rSTATE)->GetValueUInt();
			if (state.iostCommandFail)
			{
				s.third = i / 10;
				s.fourth = (STATUS_FOURTH)(i % 10);
				_fails[countFail++].status = s;
			}
		}
	}
	...
	for(UINT c = 0; c < _config->GetAllChannels().size(); c++)
	{
		...
		if (ch->GetType() == ioLogicalIn || ch->GetType() == ioLogicalOut || ch->GetType() == ioArincWordOut ||
				ch->GetType() == ioCheck || ch->GetType() == ioCommand ||
				ch->GetType() == ioDigitalIn || ch->GetType() == ioDigitalOut)
			continue;
		IOSTATE lstate;
		...
		STATUS s;
		auto gr = ch->GetRegister(REGISTER_ID::rGROUP)->GetValueUInt();
		...
		if(lstate.iostAnalogNoSignal == 1)
			...
			s.group.fourth = static_cast<STATUS_FOURTH>(ch->GetCheckAlarmCode());
