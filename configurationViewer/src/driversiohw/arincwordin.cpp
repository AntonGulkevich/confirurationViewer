...
{
	auto periodMax=  (_periodMax * 3) < 100 ? 100 : (_periodMax * 3);
	_state.iostArincWNoWord = _channel->GetTimeout(_label, false) > periodMax ? 1 : 0;
	if (_state.iostArincWNoWord == 1)
		_value = 0;
	else
		_value = static_cast<float>(_channel->GetValue(_label, false) & 0xFFFFFF00); // Отрезаем адрес т.к. float 24 бита
	_state.iostArincChNoSignal = _channel->GetState().iostArincChNoSignal;

}

bool ArincWordIn::IsDataParity() const
{
