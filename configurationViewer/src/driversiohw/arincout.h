...
private:
	...
	UINT GetFifoStatus() const	{ return UCU_IORD_32DIRECT(_baseAddress, FIFO_STATUS_OFFSET); }
	bool IsSignalTimeOut() const	{ return ((UCU_IORD_32DIRECT(_baseAddressControl, 0x10) >> _selfNumber) & 1) == 1; }
...
public:
	...
	Arinc::ParityOut GetParity() const	{ return static_cast<Arinc::ParityOut>(_controlRegister.parity); }
	...
	void SetMode(Mode mode) { _mode = mode; UCU_IOWR_32DIRECT(_baseAddress, MODE_OFFSET, static_cast<UINT>(mode)); }
	void ClearFifo() { UCU_IOWR_32DIRECT(_baseAddress, CLEAR_FIFO_OFFSET, 1); }

	void SendWord(Arinc::WORD429 data);
	UINT GetSelfNum() const	{ return _selfNumber; }
