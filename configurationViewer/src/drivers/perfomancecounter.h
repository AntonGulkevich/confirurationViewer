...
public:
	...
	float GetLoadBg() { return static_cast<float>(_totalBG) / (GetValue() - _lastReset) * 100.0; }
	float GetLoadLogic() { return static_cast<float>(_totalLogic) / (GetValue() - _lastReset) * 100.0; }
	float GetLoadDiag() { return static_cast<float>(_totalDiag) / (GetValue() - _lastReset) * 100.0; }
	float GetLoadIrq() { return static_cast<float>(_totalIrq) / (GetValue() - _lastReset)* 100.0; }
