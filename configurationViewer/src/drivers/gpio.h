...
public:
	...
	explicit Gpio(UINT deviceId) : _deviceId(deviceId) {}
	virtual ~Gpio() {}

	void Init() const;

	UINT GetInput(UINT number) const;
	void SetOutput(UINT number, UINT value) const;

	void SetLED(LEDS led, bool isOn) { SetOutput(static_cast<UINT>(led), isOn ? 0 : 1); }
	void SetCMWriteProtect(bool isOn) { SetOutput(35, isOn ? 1 : 0); }
	static void SVSet()
	{
		svSetValue = 1 - svSetValue;
		//SetOutput(8, svSetValue);
		//SetOutput(9, svSetValue);
	}
