...
void FWLoader::LoadFW()
{
	...
	auto status = _usb->WaitForStatus(UsbTr::All);
	if (!(status & UsbTr::SomeError))
	{
		auto totalPacket = _usb->GetTotalPacketCount();
		auto data = new BYTE[PACKET_SIZE * totalPacket];
		auto isOk = true;
		CRC32 crc32;
		while(totalPacket > currentPacket)
		{
			...
			if (!(status == UsbTr::SomeError))
			{
				...
				auto cp = _usb->GetCurrentPacketNumber();
				_display->SetTextByValue(totalPacket-cp, false);
				auto crc = _usb->GetCRC();
				if (currentPacket == cp)
				{
					crc32.Reset();
