...
bool CalibrationManager::CheckCalibration(DriversIOManager::Calibration& calib) const
{
	CRC32 crc;
...
void CalibrationManager::CalcAvgSCT(int number, int regNum)
{
	...
	if (regNum == 1)
		_calibration.SCT[number].offsetSin = ROUND(sumReg * 4 / (float)traceLength);  // Умножаем на 4 потому что ЦАП 16 бит а АЦП 12 бит
	else if (regNum == 2)
		_calibration.SCT[number].offsetCos = ROUND(sumReg * 4 / (float)traceLength);
}
...
void CalibrationManager::ProcessCalibration()
{
	...
	if (localStage == 0x00)
	{
		...
		case 7: // Установка значений для калибровок ЦАП Sin
			...
			_drivers->GetDisplay()->SetTextByText("CL_7", true);
			break;
		...
		case 9:
			...
			_drivers->GetDisplay()->SetTextByText("CL_8", true);
			break;
