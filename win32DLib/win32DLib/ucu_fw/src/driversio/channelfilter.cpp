...
void ChannelFilter::CreateFilterRegisters() const
{
	_channel->registers_t[static_cast<UINT>(REGISTER_ID::rFILTRTYPE)] = {REGISTER_ID::rFILTRTYPE, rwConstant, rtDec, 0, 2, 0, false};
	_channel->registers_t[static_cast<UINT>(REGISTER_ID::rFILTRBASE)] = {REGISTER_ID::rFILTRBASE, rwConstant, rtDec, 1, 250.0f, 1, false};
	_channel->registers_t[static_cast<UINT>(REGISTER_ID::rNOTFILTREDVALUE)] = {REGISTER_ID::rNOTFILTREDVALUE, rwVariable, rtFloat, MIN_FLOAT, MAX_FLOAT, 0.0f, false};
	_channel->registers_t[static_cast<UINT>(REGISTER_ID::rFILTRREADY)] = {REGISTER_ID::rFILTRREADY, rwVariable, rtDec, 0, 1, 0, false};
}

void ChannelFilter::CreateFilterBuffer()
{
	filterType = static_cast<FILTR_TYPE>(_channel->GetRegister(REGISTER_ID::rFILTRTYPE)->GetValueUInt());
	switch(filterType)
	{
...
void ChannelFilter::NormalizeBuffer(float breakPoint) const
{
...
//	const float gain_koeff[3] =  { 1.0f / 0.981486f, 1.0f / 0.996807f};
	...
	auto ft = static_cast<UINT>(filterType);
