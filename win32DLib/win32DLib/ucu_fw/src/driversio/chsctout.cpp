...
ChSCTOut::ChSCTOut(CPattern* const pattern, UINT number) : IChannelOut(pattern)
{
	...
	registers_t[static_cast<UINT>(REGISTER_ID::rVALUE)] = {REGISTER_ID::rVALUE, rwVariable, rtFloat, MIN_FLOAT, MAX_FLOAT, 0.0f, true};
	registers_t[static_cast<UINT>(REGISTER_ID::rCONTROL)] = {REGISTER_ID::rCONTROL, rwVariable, rtDec, 0.0f, 0xFFFF, 0.0f, true};
	registers_t[static_cast<UINT>(REGISTER_ID::rSTATE)] = {REGISTER_ID::rSTATE, rwConstant, rtDec, 0.0f, 0xFFFFF, 0.0f, false};
	...
	registers_t[static_cast<UINT>(REGISTER_ID::rDELAY)] = {REGISTER_ID::rDELAY, rwUser, rtDec, 0.0f, 360.0f, 0.0f, true};
	registers_t[static_cast<UINT>(REGISTER_ID::rTYPE)] = {REGISTER_ID::rTYPE, rwConstant, rtDec, 0.0f, 1.0f, 0.0f, true};
...
{
	_channel->SetType(static_cast<SCTOut::SCTType>(registers_t[static_cast<UINT>(REGISTER_ID::rTYPE)].reg->GetValueUInt()));
	_channel->SetPhaseShift(registers_t[static_cast<UINT>(REGISTER_ID::rDELAY)].reg->GetValueUInt());
	UpdateDataToHW();
}

void ChSCTOut::UpdateDataToHW()
{
	auto value = registers_t[static_cast<UINT>(REGISTER_ID::rVALUE)].reg->GetValueFloat() + registers_t[static_cast<UINT>(REGISTER_ID::rDELTA)].reg->GetValueFloat();
...
{
	registers_t[static_cast<UINT>(REGISTER_ID::rSTATE)].reg->SetValue(static_cast<UINT>(_channel->GetState().dword));
