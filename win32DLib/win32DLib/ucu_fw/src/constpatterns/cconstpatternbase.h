...
#define CONVERT(name) { if (cachedIO[name] != nullptr) cachedIO[name]->Convert(); else TRACEUCU("Конвертируется не кэшированый IO\r"); }
#define IS_CONNECTED(name) (cachedIO[name] != nullptr)
...
public:
    ...
    CRegister* GetReg(UINT name, REGISTER_ID reg) const;
...
protected:
	...
	void CacheIO(IChannel*& io, IOTYPES type, UINT num) const;
	void CacheIOLabel(IChannel*& io, IOTYPES type, UINT num, BYTE label) const;
	IChannel* AddWord(IOTYPES type, UINT num, BYTE label) const;
