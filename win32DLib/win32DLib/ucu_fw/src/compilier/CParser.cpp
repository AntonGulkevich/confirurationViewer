...
short CParser::IndexOfList(const char* llist[]) const
{
	for (auto i = 0; strlen(llist[i]) != 0; i++)
...
int CParser::PopSeparator()
{
	...
	stack[stackPtr] = 0;
	auto res = DefineSeparator(stack);
	stackPtr = 0;
	return res;
}
...
void CParser::PrintTokenFlow() const
{
...
{
	short state = 0;
	auto k = 0; // Индекс для движения по стеку
	auto existWarning = 0;
	TOKEN t;
	auto curT = GetNextToken();
...
int CParser::WriteResult(int res) const
{
	switch(res)
	{
	case prOk:
...
void CParser::WriteError(TOKEN t, const char* format, ...) const
{
    //_console->Write("\x1B[0;7;31m");
    _console->Write("Error: ");
    //_console->Write("\x1B[0;0;30;47m");
	...
	vsprintf(static_cast<char*>(buff), format, list);
	va_end(list);
#ifdef _UCU_EMULATION	
	_console->Write(buff);
...
void CParser::WriteCriticalWarning(TOKEN t, const char* format, ...) const
{
    //_console->Write("\x1B[1;7;31;47m");
    _console->Write("Critical Warning: ");
    //_console->Write("\x1B[0;0;30;47m");
...
void CParser::WriteWarning(TOKEN t, const char* format, ...) const
{
