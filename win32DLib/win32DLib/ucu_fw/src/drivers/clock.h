...
// Чтение данных из микросхемы часов
    void ReadData(void* buff, DWORD address, UINT count) const;
...
public:
	...
	// Запись данных в микросхему часов
	void WriteData(void* buff, DWORD address, UINT count) const;
	// Чтение данных из микросхемы часов
	void ReadData(void* buff, DWORD address, UINT count) const;

	// Получение даты в текстовом формате
	void GetDateStr(char* buff, const CHIPTIME* ct = nullptr);
	// Получение времени в текстовом формате
	void GetTimeStr(char* buff, const CHIPTIME* ct = nullptr);
	// Получение даты и времени в текстовом формате
	void GetDateTimeStr(char* buff, const CHIPTIME* ct = nullptr);
	...
	UINT GetCurrentTime() const	{ return _currentTime; }
