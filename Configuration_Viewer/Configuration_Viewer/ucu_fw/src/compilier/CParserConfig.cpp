...
int CParserConfig::AddAction(RULES rule)
{
	...
	case RULE_CONNECTION___DESTINATION_EQ_SOURSE_SEMI:
		...
		if (!((dest.dt == dtOut && !_driversIO->IsIOExist(dest.type, dest.pinNum)) ||
			...
			(dest.dt == dtPatternConst && config->GetPattern(IndexOfDevice(dest.patternNum))->GetInput(dest.type, dest.pinNum) == nullptr) ||
			(src.dt == dtPatternConst && config->GetPattern(IndexOfDevice(src.patternNum))->GetOutput(src.type, src.pinNum) == nullptr)))
		{
	...
	case RULE_SOURSE___IN_POINT_INNAME_POINT_DECLITERAL:
		...
		{
            WriteCriticalWarning(params[0],
			"Вход %s.%d несуществует", GetTypeStr(src.type), static_cast<BYTE>(src.pinNum + 1));
			existWarning = 1;
		}
		break;
	case RULE_SOURSE___DEVOUT_POINT_DECLITERAL_POINT_OUTNAME_POINT_DECLITERAL:
		...
		if (IndexOfDevice(src.patternNum) == -1)
		{
			...
			{
				config->AddPattern();
				config->GetPattern(static_cast<BYTE>(deviceNums.size()))->SetDeviceNum(src.patternNum);
	...
	case RULE_SOURSE___DEVCONSTOUT_POINT_DECLITERAL_POINT_OUTNAME_POINT_DECLITERAL:
		...
		if (config->GetPattern(IndexOfDevice(src.patternNum))->GetOutput(src.type, src.pinNum) == nullptr)
		{
            WriteCriticalWarning(params[0],
			"Выход %s.%d библиотечного устройства %d несуществует", GetTypeStr(src.type), static_cast<BYTE>(src.pinNum + 1), 0xF0 - src.patternNum);
			existWarning = 1;
		}
		break;
	case RULE_DESTINATION___OUT_POINT_OUTNAME_POINT_DECLITERAL:
		...
		{
            WriteCriticalWarning(params[0],
			"Выход %s.%d несуществует", GetTypeStr(dest.type), static_cast<BYTE>(dest.pinNum + 1));
			existWarning = 1;
		}
		break;
	case RULE_DESTINATION___DEVIN_POINT_DECLITERAL_POINT_INNAME_POINT_DECLITERAL:
		...
		if (IndexOfDevice(dest.patternNum) == -1)
		{
			...
			{
				config->AddPattern();
				config->GetPattern(static_cast<BYTE>(deviceNums.size()))->SetDeviceNum(dest.patternNum);
	...
	case RULE_DESTINATION___DEVCONSTIN_POINT_DECLITERAL_POINT_INNAME_POINT_DECLITERAL:
		...
		if (config->GetPattern(IndexOfDevice(dest.patternNum))->GetInput(dest.type, dest.pinNum) == nullptr)
		{
            WriteCriticalWarning(params[0],
			"Вход %s.%d библиотечного устройства %d несуществует", GetTypeStr(dest.type), static_cast<BYTE>(dest.pinNum + 1), 0xF0 - dest.patternNum);
			existWarning = 1;
		}	
		break;
	...
	case RULE_PROGSTEP___ID_EQ_DECLITERAL_SEMI:
		if ((reg = config->GetRegister(static_cast<REGISTER_ID>(params[0].data))) == nullptr)
        {
            WriteCriticalWarning(params[0],
			"Регистр %s для секции GENERAL не существует", registers[params[0].data]);
            existWarning = 1;
        }
		else if (config->GetRegisterT(static_cast<REGISTER_ID>(params[0].data))->type == rtString)
        {
            WriteCriticalWarning(params[0],
		...
		{
			auto regT = config->GetRegisterT(static_cast<REGISTER_ID>(params[0].data));
			if (regT == nullptr || ((regT->min_value > params[2].dec) || regT->max_value < params[2].dec))
			{
				WriteCriticalWarning(params[0],
 ...
 CParserConfig::SYMBOLS CParserConfig::GetIdKeyWord(int id) const
 {
	switch(id)
	{
	case 0:
...
CParserConfig::SYMBOLS CParserConfig::GetTokenSymbol(CParser::TOKEN t) const
{
	switch(t.tc)
	{
