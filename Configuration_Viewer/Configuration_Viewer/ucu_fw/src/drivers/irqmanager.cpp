...
#else
PerfomanceCounter* IrqManager::_pc = nullptr;
...
{
	_pc->IrqStart();
	auto tmp = reinterpret_cast<CIrqContext*>(ptr);
	tmp->obj->OnIrq(tmp->param);
	_pc->IrqStop();
}

void IrqManager::RegIRQ(DWORD irq, IUsingIrq* pobj, DWORD param) const
{
#ifndef EM_DEBUG
		if (irq < interruptsCount)
	{
...
void IrqManager::RegIRQ(DWORD irq, Xil_ExceptionHandler func, void* context) const
{
#ifndef EM_DEBUG 
	XScuGic_Connect(&_instance, irq,(Xil_ExceptionHandler)func,	context);
#endif

}


void IrqManager::EnableIRQ(DWORD irq) const
{
#ifndef EM_DEBUG 
	XScuGic_Enable(&_instance, irq);
#endif
...
void IrqManager::DisableIRQ(DWORD irq) const
{
#ifndef EM_DEBUG
	XScuGic_Disable(&_instance, irq);
#endif
...
void IrqManager::DisableAll() const
{
	for (UINT i = 0; i < interruptsCount; i++)
		if (contexts[i].obj != 0)
			DisableIRQ(i);
}
void IrqManager::EnableAll() const
{
	for (UINT i = 0; i < interruptsCount; i++)
		if (contexts[i].obj != 0)
			EnableIRQ(i);
...
void IrqManager::ClearPeripheralIrq(DWORD IntID) const
{
	/*
	* Write to the EOI register, we are all done here.
	* Let this function return, the boot code will restore the stack.
