...
void I2C::Write(const void* buffer, DWORD address, UINT size)
{
...
void I2C::Write(const void* buffer, DWORD address, UINT size)
{
	...
	if (_isDeviceAvailable)
	{
		...
		_iicInstance.write(static_cast<const char *>(buffer), size*sizeof(char));
	}
	_iicInstance.close();
}

void I2C::setFileName(const std::string& fileName_)
{	
	fileName = fileName_;
}

std::string I2C::getFileName()
{
	return fileName;
}
#endif
