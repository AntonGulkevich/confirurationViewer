...
void Display::SetPoint(POINTS point, bool val)
{
	points[static_cast<UINT>(point)] = val;
}
...
void Display::SetValue(const BYTE* text)
{
	for(auto i = 0; i < 4; i++)
		value[i] = text[i];
	points[0] = points[1] = points[2] = points[3] = false;
...
BYTE Display::GetByteByDigit(BYTE digit) const
{
	if (digit > 15)
		return 0x08;
	else
...
BYTE Display::GetByteByLetter(char ch) const
{
	switch (ch)
	{
	case 0:
	...
	case 14:
	case 15:
		return GetByteByDigit(static_cast<BYTE>(ch));
	case '0':
	case '1':
	case '2':
	case '3':
	...
	case '8':
	case '9':
		return GetByteByDigit(static_cast<BYTE>(ch - '0'));
	case 'A':
	case 'a':
	case 'А': // рус
	case 'а': // рус
