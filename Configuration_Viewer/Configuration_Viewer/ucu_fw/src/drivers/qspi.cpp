...
void Qspi::ReadPage(void* buffer, int offset, int address, int count)
{
	...
	_writeBuffer[COMMAND_OFFSET] = READ_CMD;
	_writeBuffer[ADDRESS_1_OFFSET] = static_cast<u8>((address & 0xFF0000) >> 16);
	_writeBuffer[ADDRESS_2_OFFSET] = static_cast<u8>((address & 0xFF00) >> 8);
	_writeBuffer[ADDRESS_3_OFFSET] = static_cast<u8>(address & 0xFF);
	...
	*/
	...
	XQspiPs_PolledTransfer(_qspiInstancePtr, &WriteEnableCmd, NULL,
		...
		*/
		UINT addr = sector << SECTOR_SIZE_LOG;
		_writeBuffer[COMMAND_OFFSET] = SEC_ERASE_CMD;
		_writeBuffer[ADDRESS_1_OFFSET] = static_cast<u8>(addr >> 16);
		_writeBuffer[ADDRESS_2_OFFSET] = static_cast<u8>(addr >> 8);
		_writeBuffer[ADDRESS_3_OFFSET] = static_cast<u8>(addr & 0xFF);
