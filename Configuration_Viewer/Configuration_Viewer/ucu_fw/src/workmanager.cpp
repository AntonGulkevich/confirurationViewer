...
DISPATH_RESULT WorkManager::ProcessFlag(Flags id)
{
	...
	case Flags::Timer1Sec:
		if (!_menu->IsActive() && !_calibrationManager->IsCalibrationInProgress())
...
bool WorkManager::validateConfig()
{
	_calibrationManager->LoadCalibration();
	_calibrationManager->ApplyCalibration();

	_commod->Init();
	auto result = _config->LoadConfig();
	if (result != CConfiguration::LOAD_RESULT::OK)
	{
		_drivers->GetDisplay()->SetTextByText("A210");
		ERROR_LOOP;
	}
	...
	return (result == CConfiguration::LOAD_RESULT::OK) ? true : false;
}
