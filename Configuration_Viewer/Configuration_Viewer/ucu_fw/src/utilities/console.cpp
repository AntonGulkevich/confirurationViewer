...
void Console::Write(const char* str) const
{
	SendData(str, false);
}

void Console::WriteLine(const char* str) const
{
	SendData(str, true);
}

void Console::Format(const char* str, ...) const
{
    char buff[2048];
    va_list list;

    va_start(list, str);
    vsprintf(static_cast<char*>(buff), str, list);
    va_end(list);
    SendData(buff, false);
}

void Console::FormatLine(const char* str, ...) const
{
    char buff[2048];
    va_list list;
