...
class Diagnostic : public IDispatchFlags, public IUsingFlags
{
...
public:
	...
	enum Markers
	{
        ...
        SetCMNumber = 0x89744024,
        SetBlockNumber = 0x6c06acf4,
        GetPassport = 0x2783335a,
        SetPassport = 0x39ac8413,
        GetMeasuredInfo = 0x6d6dcc0b,
        PCCommands = 0x3df3409,
        PCData = 0x5ce23f8,
	};
	...
	struct DiagDataAnalog
	{
		...
		BYTE mainAdcWrchn;
		BYTE reserv1;
		DWORD analogPacketCount;
		DWORD digitalPacketCount;
		DWORD analogErrorCount;
		DWORD endMarker;
	};
	...
	struct ChannelRegistersData
	{
		...
		BYTE channelNum;
		BYTE count;
		BYTE data[sizeof(RegisterData) * static_cast<UINT>(REGISTER_ID::COUNTREGISTERS)];
	};
...
private:
	...
	IChannel* _measuringChannel;
	int _measuringPointIndex;

	void UnpackChannelsSettings(const ChannelsSettings* data);
	RS::ParityType ConvertRSParity(UINT p);
	void ParsePacketData();
	void ProcessCommands(Commands cmd);
	void ProcessData(const BYTE* _inDataBuffer, UINT size);

	// Разбор входящего буфера от ПК
