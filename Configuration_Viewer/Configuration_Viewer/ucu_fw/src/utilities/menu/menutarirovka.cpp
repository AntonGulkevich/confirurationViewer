...
MenuTarirovka::MenuTarirovka(ChannelCalibration* channel, UserSettingManager* settings, Gpio* gpio, const char* name, const BYTE points) : MenuItem(name, points), _channel(channel), _settings(settings), _gpio(gpio)
{
...
UINT MenuTarirovka::CheckTarirovka()
{
	...
	if (res == 0)
	{
    	...
    	if(res == 0)
        {
    		...
    		_gpio->SetCMWriteProtect(false);
    		_settings->Save();
    		_gpio->SetCMWriteProtect(true);
#ifdef _TAR_TRACE
