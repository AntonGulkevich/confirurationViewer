...
ChArincWordIn::ChArincWordIn(CPattern* const pattern, ChArincIn* patternChannel, UINT number) : IChannelIn(pattern), _patternChannel(patternChannel)
{
	...
	registers_t[static_cast<UINT>(REGISTER_ID::rBITSCOUNT)] = 	{REGISTER_ID::rBITSCOUNT, rwConstant, rtDec, 1, 21, 20.0f, false};
	registers_t[static_cast<UINT>(REGISTER_ID::rRANGE)] = 	{REGISTER_ID::rRANGE, rwConstant, rtFloat, MIN_FLOAT, MAX_FLOAT, static_cast<float>(1<<20), false};
...
{
	UpdateDataToHW();
	_channel->SetPeriodRange(GetRegister(REGISTER_ID::rPERIODMIN)->GetValueUInt(), GetRegister(REGISTER_ID::rPERIODMAX)->GetValueUInt());
}

void ChArincWordIn::UpdateDataToHW()
{
	_dataMask = _channel->IsDataParity() ? 0xFFFFFF00 : 0x7FFFFF00;
	if (registers_t[static_cast<UINT>(REGISTER_ID::rID)].reg->IsFilled())
...
{
	auto val = ftou__(_channel->GetValue());
	registers_t[static_cast<UINT>(REGISTER_ID::rVALUE)].reg->SetValue(static_cast<UINT>(val & _dataMask));
	registers_t[static_cast<UINT>(REGISTER_ID::rMATRIX)].reg->SetValue((val >> 29) & 0x3);
...
bool ChArincWordIn::IsTimeouted() const
{
