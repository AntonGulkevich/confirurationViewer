...
ChArincIn::ChArincIn(CPattern* const pattern, UINT number) : IChannelIn(pattern)
{
	...
	registers_t[static_cast<UINT>(REGISTER_ID::rPARITYTYPE)] = {REGISTER_ID::rPARITYTYPE, rwConstant, rtDec, 0.0f, 2.0f, 0.0f, false};
	CreateRegisters();
...
/*	state.iostArincWNoWord = 0;
	...
	registers_t[static_cast<UINT>(REGISTER_ID::rSTATE)].reg->SetValue(static_cast<UINT>(state.dword));
