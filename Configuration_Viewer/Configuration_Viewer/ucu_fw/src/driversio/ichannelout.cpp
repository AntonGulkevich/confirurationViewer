...
{
	if (IS_IN(ioType))
		return reinterpret_cast<IChannel*>(static_cast<ChArincIn*>(_pattern->GetInput(ioType, num))->GetLabel(label));  // Добавить чтение по ID
	else if (IS_OUT(ioType))
		return reinterpret_cast<IChannel*>(static_cast<ChArincOut*>(_pattern->GetOutput(ioType, num))->GetLabel(label));  // Добавить чтение по ID;
	return nullptr;
}
