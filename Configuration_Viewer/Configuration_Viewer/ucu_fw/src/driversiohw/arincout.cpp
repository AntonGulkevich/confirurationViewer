...
{
	if ((GetFifoStatus() & static_cast<UINT>(Arinc::FifoStatus::FifoFull)) == 0)
	{
	...
	{
		data.time = wordPauses[static_cast<UINT>(_controlRegister.rate)];
		_wordWaitmks -= wordPauses[static_cast<UINT>(_controlRegister.rate)];
	}
