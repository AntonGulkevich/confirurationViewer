...
public:
	...
	void SetMaximumSignalLevel(DWORD data) { UCU_IOWR_32DIRECTF(_baseAddress, _inputOffset + maxLevelOffset, data); }
	//0x180 Ц 0x194 min signal lvl (w/r) 32bit - ћинимальный уровень сигнала
	void SetMinimumSignalLevel(DWORD data) { UCU_IOWR_32DIRECTF(_baseAddress, _inputOffset + minLevelOffset, data); }
	//0x80 Ц 0x94 data sigma (r) «начение —ƒјѕ÷ (24b)
	void UpdateValue();
	//0xC0 Ц 0xD4 data float (r) 32 bit - –ассчитанное значение, % (float)
	DWORD GetRegister() const	{ return UCU_IORD_32DIRECT(_baseAddress, _inputOffset + registerOffset);  }

	static void SetRef(RefType type) { UCU_IOWR_32DIRECT(_staticBaseAddress, refControlOffset, static_cast<UINT>(type)); }
