...
void SCTOut::SetType(SCTType type)
{
	...
	if (channelNum < 3)
		_control->SetDelay(560 + channelNum * AnalogIn::delayStep); // 560 + номер канала * delayStep
	else
		_control->SetDelay(600 + (channelNum-3) * AnalogIn::delayStep); // 560 + номер канала * delayStep
