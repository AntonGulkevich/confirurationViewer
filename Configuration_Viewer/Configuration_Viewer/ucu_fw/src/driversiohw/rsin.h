...
private:
	...
	UINT GetCount() const	{ return UCU_IORD_32DIRECT(_baseAddress, COUNT_OFFSET); }
	UINT GetCountPT() const	{ return UCU_IORD_32DIRECT(_baseAddress, COUNTPT_OFFSET); }
...
public:
	explicit RSIn(DWORD baseAddress);
	virtual ~RSIn() {}


	void SetBaudRate(RS::BaudRate rate) { UCU_IOWR_32DIRECT(_baseAddress, BAUDRATE_OFFSET, (50000000 / static_cast<UINT>(rate))-1); }
	RS::BaudRate GetBaudRate() const	{ return static_cast<RS::BaudRate>((50000000 / UCU_IORD_32DIRECT(_baseAddress, BAUDRATE_OFFSET)) + 1); }
	...
	void SetWorkMode(WorkMode mode) { _workMode = mode; UCU_IOWR_32DIRECT(_baseAddress, WORKMODE_OFFSET, static_cast<UINT>(mode)); }
	...
	// Диагностика
	UINT GetBufferSize() const	{ return _receivedCount; }
