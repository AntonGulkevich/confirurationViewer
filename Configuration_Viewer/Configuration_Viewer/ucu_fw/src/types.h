...
union IOSTATE
{
	struct
	{
		...
		UINT iostAnalogSingleNPTDACFail: 1; // Одиночный отказ отклониение от контрольного для НПТ
...
enum class Flags
{
	...
	DataPCParse, // Продолжение разбора данных если еще осталось что то в буфере
	Timer1Sec, // Часы
	MeasurmentComplete, // Закончено измерение для тарировки
	Count,
