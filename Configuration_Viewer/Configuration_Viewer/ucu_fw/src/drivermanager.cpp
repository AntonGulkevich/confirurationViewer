...
#ifdef EMULATION 
std::string DriverManager::getI2cCommodFileName()
{
	return _i2cCommod.getFileName();
}

void DriverManager::setI2cCommodFileName(const std::string commodFileName)
{
	_i2cCommod.setFileName(commodFileName);
}
#endif
...
void DriverManager::WriteEEPROM(void* buff, UINT address, UINT count)
{
	...
	{
		auto startAddress = address % pageSize;
		UINT offset = 0;
		if (startAddress > 0)
		{
		...
		for(; static_cast<int>(offset) < static_cast<int>(count-pageSize); offset += pageSize)
			_i2cEEPROM.Write(&reinterpret_cast<BYTE*>(buff)[offset], address + offset, pageSize);
		if (static_cast<int>(offset - count) > 0)
