...
DriversIOManager::DriversIOManager()
{
	...
	RegIRQ(static_cast<DWORD>(XPAR_FABRIC_MAIN_ADC_HUB_0_IRQ_INTR));

	_calibrate = new Calibrate(static_cast<DWORD>(XPAR_MAIN_ADC_HUB_0_BASEADDR));
	for(UINT i = 0; i < analogInCount; i++)
	{
		_analogIns[i] = new AnalogIn(static_cast<DWORD>(XPAR_MAIN_ADC_HUB_0_BASEADDR), i*4);
		_inputs.push_back(_analogIns[i]);
	}
	for(UINT i = 0; i < SCTOutCount; i++)
	{
		_SCTControls[i] = new AnalogIn(static_cast<DWORD>(XPAR_MAIN_ADC_HUB_0_BASEADDR), (i+13)*4);
		_inputs.push_back(_SCTControls[i]);
	}
	for(UINT i = 0; i < NPTOutCount; i++)
	{
		_NPTControls[i] = new NptControl(static_cast<DWORD>(XPAR_MAIN_ADC_HUB_0_BASEADDR), (i+19)*4);
		_inputs.push_back(_NPTControls[i]);
	}
	for(UINT i = 0; i < potentiometrCount; i++)
	{
		_potentiometrs[i] = new Potentiometr(static_cast<DWORD>(XPAR_POT_TOP_0_BASEADDR), i*4);
		_inputs.push_back(_potentiometrs[i]);
	}
	for(UINT i = 0; i < descreeteInputCount; i++)
	{
		_descreeteInputs[i] = new DescreeteInput(static_cast<DWORD>(XPAR_EX_MAX_0_BASEADDR), i*4);
		_inputs.push_back(_descreeteInputs[i]);
	}
	for(UINT i = 0; i < SCTOutCount; i++)
	{
		_SCTOuts[i] = new SCTOut(static_cast<DWORD>(XPAR_RESOLV_TOP_0_BASEADDR), i*4, _SCTControls[i]);
		_outputs.push_back(_SCTOuts[i]);
	}
	for(UINT i = 0; i < NPTOutCount; i++)
	{
		_NPTOuts[i] = new NPTOut(static_cast<DWORD>(XPAR_CURRENT_DAC_TOP_0_BASEADDR), i*4, _NPTControls[i]);
		_outputs.push_back(_NPTOuts[i]);
	}
	for(UINT i = 0; i < descreeteOutputCount; i++)
	{
		_descreeteOutputs[i] = new DescreeteOutput(static_cast<DWORD>(XPAR_EX_MAX_0_BASEADDR), i);
...
void DriversIOManager::CalculateBreak()
{
	for(UINT i = 0; i < analogInCount; i++)
		_analogIns[i]->CalcBreak();
	for(UINT i = 0; i < potentiometrCount; i++)
		;//_potentiometr[i]->CalcBreak();
}
...
bool DriversIOManager::IsIOExist(IOTYPES ioType, UINT num) const
{
	switch (ioType) {
	case ioDigitalIn:
