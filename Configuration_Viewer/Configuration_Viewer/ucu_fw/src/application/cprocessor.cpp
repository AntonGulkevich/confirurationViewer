...
#include "../driversio/channelfilter.h"
#include "../driversio/channelrate.h"
...
void CProcessor::Instance()
{
	for(actionPtr = 0; actionPtr < actionsTr.size(); actionPtr++)
	{
		...
		{
			actionsTr[actionPtr].ptrs.ptr2 = reinterpret_cast<DWORD>(_channel->GetRegister(static_cast<REGISTER_ID>(actionsTr[actionPtr].ptrs.ptr2)));
		}
			break;
		...
		{
			IChannel* io = nullptr;
			auto ct = static_cast<IOTYPES>((actionsTr[actionPtr].ptrs.ptr1 >> 8) & 0xFF);
			BYTE cn = static_cast<IOTYPES>((actionsTr[actionPtr].ptrs.ptr1) & 0xFF);
			io = _channel->GetPatternChannel(ct, cn);
			if (io!=nullptr)
			{
				actionsTr[actionPtr].ptrs.ptr1 = reinterpret_cast<DWORD>(io);
				actionsTr[actionPtr].ptrs.ptr2 = reinterpret_cast<DWORD>(io->GetRegister(static_cast<REGISTER_ID>(actionsTr[actionPtr].ptrs.ptr2)));
			} else
			{
        ...
        case acReadRegisterLabel:
		{
			...
			auto ct = static_cast<IOTYPES>((actionsTr[actionPtr].ptrs.ptr1 >> 8) & 0xFF);
			BYTE cn = static_cast<IOTYPES>((actionsTr[actionPtr].ptrs.ptr1) & 0xFF);
			io = _channel->GetPatternChannelLabel(ct, cn, static_cast<BYTE>(actionsTr[actionPtr].ptrs.ptr1 >> 16), static_cast<BYTE>(actionsTr[actionPtr].ptrs.ptr1 >> 24));
			if (io!=nullptr)
			{
    ...
    if (!constant)
	{
		...
		for(actionPtr = 0; actionPtr < actionsTr.size(); actionPtr++)
        {
            ...
            case acMul:
                ...
                break;
            case acIntMul:
                reg3 = static_cast<UCU_FLOAT>(ftoi__(reg1) % ftoi__(reg2));
                break;
            case acDiv:
                reg3 = reg1 / reg2;
                break;
            case acMinus:
                ...
                break;
            case acNot:
                reg3 = static_cast<UCU_FLOAT>(!ftoi__(reg1));
                break;
            case acGtEq:
                reg3 = reg1 >= reg2;
                break;
            ...
            case acFunction:
            {
                ...
                for(auto i = ftoi__(id.param2 - 1); (i >= 0 && exprPtr); i--)
                {
                    funcParams[i] = exprPop();
                }
                switch(static_cast<FUNCTION_ID>(ftou__(id.param1)))
                {
                case fLINEAR:  // 3 параметра
                {
                	...
                	auto io = _channel->GetPatternChannel(static_cast<IOTYPES>(ftou__(funcParams[1])), static_cast<UINT>(funcParams[2]));
                    if (io != nullptr && io->IsHasCalibration())
                        reg3 = io->GetChCalibration()->GetValueByKoeff(static_cast<float>(funcParams[0]));
                    else
                        reg3 = 0;
                }
                    break;
                case fRESETFILTR:
                {
                	...
                	auto io = _channel->GetPatternChannel(static_cast<IOTYPES>(ftou__(funcParams[1])), static_cast<UINT>(funcParams[0]));
                    if (io != nullptr && io->IsValueFiltered())
        		...
        		{
                	auto io = _channel;
                    if (io != nullptr && io->IsRateCalculating())
...
/*
                ...
                case fTOBNR:  // 3 параметра
                    {
                    	...
                    	// 3 - Диапазон
                        reg3 = static_cast<UCU_FLOAT>(ToBNR(funcParams[0], ftou__(funcParams[1]), funcParams[2]));				
        			}
                    break;
                ...
                case fTORTM:  // 3 параметра
                    {
                        ...
                        reg3 = static_cast<UCU_FLOAT>(ToRTM(funcParams[0], ftou__(funcParams[1]), funcParams[2]));				
                    }
                    break;
                ...
                case fSHOWALARM: // 3 параметра
                    ...
                    break;
                case fSETBIT:
                    reg3 = static_cast<UCU_FLOAT>(SetBit(ftou__(funcParams[0]), ftou__(funcParams[1]), ftou__(funcParams[2])));
                    break;
                case fGETBIT:
                    reg3 = static_cast<UCU_FLOAT>(GetBit(ftou__(funcParams[0]), ftou__(funcParams[1])));
                    break;
                case fSETFIELD:
                    reg3 = static_cast<UCU_FLOAT>(SetField(ftou__(funcParams[0]), ftou__(funcParams[1]), ftou__(funcParams[2]), ftou__(funcParams[3])));
                    break;
                case fGETFIELD:
                    reg3 = static_cast<UCU_FLOAT>(GetField(ftou__(funcParams[0]), ftou__(funcParams[1]), ftou__(funcParams[2])));
                    break;
                case fGETUCUSTATE:
                    reg3 = static_cast<UCU_FLOAT>(_channel->GetPattern()->GetConfig()->GetState().dword);
                    break;
                case fSUPPRESSALARM:
                    {
                    	auto io = _channel->GetPatternChannel(static_cast<IOTYPES>(ftou__(funcParams[1])), ftou__(funcParams[2]));
						if (io != nullptr)
							io->SetSuppressAlarm((funcParams[0] != 0));
                    reg3 = 0;
                    }
				case fSIN:
					reg3 = static_cast<UCU_FLOAT>(m_sin(funcParams[0]));
					break;
				case fCOS:
					reg3 = static_cast<UCU_FLOAT>(m_cos(funcParams[0]));
					break;
...
void CProcessor::GetDependentChannels(std::vector<IChannel*>& channels)
{
	for(actionPtr = 0; actionPtr < actionsTr.size(); actionPtr++)
	{
        ...
        case acReadRegisterLabel:
        {
		       auto io = reinterpret_cast<IChannel*>(actionsTr[actionPtr].ptrs.ptr1);
...
void CProcessor::Compile()
{
	...
	actionPtr = 0;
	actionsTr.clear();
	auto actToAct = new UINT[actionsV.size()+1]; 
	for(actionPtr = 0; actionPtr < actionsV.size(); actionPtr++)
	{      
		actToAct[actionPtr] = actionsTr.size();
		ACTION_TRIADA acTr;
		switch(static_cast<ACTIONS>(actionsV[actionPtr]))
		{
		...
		case acNot:
			...
			else if (paramsV[paramPtr] > 0)
			{
				...
				acTr.param1 = static_cast<float>(paramsV[paramPtr]-1);
			}
			else 
			{
...
#endif                        
			acTr.act = static_cast<ACTIONS>(actionsV[actionPtr]);
			...
			} else
			{
				...
				acTr.param1 = static_cast<float>(paramsV[paramPtr+1]-1);
			}
			actionsTr.push_back(acTr);
			paramPtr += 2;
#ifdef COMPILIER_TRACE
        ...
        Console::Trace("Матвыражение %d, %d, %d, %d\r\n", actionsV[actionPtr], paramsV[paramPtr], paramsV[paramPtr+1], paramsV[paramPtr+2]);
			...
			if (paramsV[paramPtr] < 0)
			{
				...
				else if (paramsV[paramPtr + 1] > 0)
				{
					...
					acTr.param2 = static_cast<float>(paramsV[paramPtr+1]-1);
				}
				else 
				{
					acTr.act = acLoadBinParamConstConst;
			...
			{
				acTr.act = acSaveParamVar;
				acTr.param1 = static_cast<float>(paramsV[paramPtr+2]-1);
			}
			actionsTr.push_back(acTr);
#ifdef COMPILIER_TRACE
			Console::TraceLine("%d %s, %g, %g, %08X, %08X", actionsTr.size(), ActionName(actionsTr.back().act), actionsTr.back().param1, actionsTr.back().param2, &actionsTr.back(), &actionsTr[0]);
...
#endif                    
			acTr.act = acJmp;
			acTr.param1 = static_cast<float>(paramsV[paramPtr]);
			actionsTr.push_back(acTr);
#ifdef COMPILIER_TRACE
			Console::TraceLine("%d %s, %g, %g, %08X, %08X", actionsTr.size(), ActionName(actionsTr.back().act), actionsTr.back().param1, actionsTr.back().param2, &actionsTr.back(), &actionsTr[0]);
#endif                                                                    
		...
		case acReadRegister:
			{
				...
				auto ct = static_cast<IOTYPES>(paramsV[paramPtr]);
				BYTE cn = paramsV[paramPtr + 1];
				if (_channel->GetPatternChannel(ct, cn) !=nullptr)
				{
					...
					{
						acTr.act = acSaveParamVar;
						acTr.param1 = static_cast<float>(paramsV[paramPtr+3]-1);
					}
					actionsTr.push_back(acTr);
#ifdef COMPILIER_TRACE
					Console::TraceLine("%d %s, %g, %g, %08X, %08X", actionsTr.size(), ActionName(actionsTr.back().act), actionsTr.back().param1, actionsTr.back().param2, &actionsTr.back(), &actionsTr[0]);
...
#endif            
				auto ct = static_cast<IOTYPES>(paramsV[paramPtr+4]);
				BYTE cn = paramsV[paramPtr];
				auto io = _channel->GetPatternChannelLabel(ct, cn, static_cast<BYTE>(paramsV[paramPtr+1]), static_cast<BYTE>(paramsV[paramPtr+3]));
				if (io != nullptr)
				{
					...
					{
						acTr.act = acSaveParamVar;
						acTr.param1 = static_cast<float>(paramsV[paramPtr+5]-1);
					}
					actionsTr.push_back(acTr);
	#ifdef COMPILIER_TRACE
					Console::TraceLine("%d %s, %g, %g, %08X, %08X", actionsTr.size(), ActionName(actionsTr.back().act), actionsTr.back().param1, actionsTr.back().param2, &actionsTr.back(), &actionsTr[0]);
		...
		case acFunction:
			...
			acTr.act = static_cast<ACTIONS>(actionsV[actionPtr]);
			acTr.param1 = static_cast<float>(paramsV[paramPtr]);
			acTr.param2 = static_cast<float>(paramsV[paramPtr+1]);
			actionsTr.push_back(acTr);
#ifdef COMPILIER_TRACE
			Console::TraceLine("%d %s, %g, %g, %08X, %08X", actionsTr.size(), ActionName(actionsTr.back().act), actionsTr.back().param1, actionsTr.back().param2, &actionsTr.back(), &actionsTr[0]);
#endif                                                        
   ...
   Console::TraceLine("%d. %s %f %f %X %X", actionPtr, name, actionsTr[actionPtr].param1,  actionsTr[actionPtr].param2, actionsTr[actionPtr].ptrs.ptr1, actionsTr[actionPtr].ptrs.ptr2);
		...
		case acJz:	
		case acJmp:
			actionsTr[actionPtr].param1 = static_cast<float>(actToAct[static_cast<UINT>(labelsV[ftou__(actionsTr[actionPtr].param1)]) & 0xFFFF]);
			break;
		default:
			break;
		}
...
BYTE CProcessor::AddVar(BYTE id)
{
	...
	varsV.push_back(id);
	return static_cast<BYTE>(varsV.size());
}
...
BYTE CProcessor::AddConst(float num)
{
	...
	return static_cast<BYTE>(constsV.size());
}
...
BYTE CProcessor::AddParam(short id)
{
	...
	return static_cast<BYTE>(paramsV.size() - 1);
}

void CProcessor::SetLabel(int labelNum)	
{ 
	labelsV[labelNum] = (static_cast<int>(actionsV.size()) &0xFFFF) + ((static_cast<int>(paramsV.size()) << 16) & 0xFFF0000);
} 

BYTE CProcessor::TestFunctionParam(int fId) const
{
...
{
	auto tmp = (((static_cast<int>(val) & 0x1FFFFF00) << 3) >> 11) ;
	tmp = tmp >> static_cast<int>(20 - count);
	return static_cast<UCU_FLOAT>(tmp) * (range  / static_cast<UCU_FLOAT>(1 << count));
      
}

UCU_FLOAT CProcessor::FromRTM(UINT val, UINT count, UCU_FLOAT range)
{
	auto tmp = (((static_cast<int>(val) & 0x3FFFFF00) << 2) >> 10) ;
	tmp = tmp >> static_cast<int>(21 - count);
	return static_cast<UCU_FLOAT>(tmp) * (range  / static_cast<UCU_FLOAT>(1 << count)); 
}
...
UINT CProcessor::ToBCD(UCU_FLOAT val, UINT fillBits) const
{
	UINT tmp = 0;
	while (val >= 1000000)
		val = val / 10;
