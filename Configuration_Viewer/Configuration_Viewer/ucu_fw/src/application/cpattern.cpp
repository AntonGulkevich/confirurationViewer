	...
	for(UINT i = 0; i < outsize; i++)
	{
		if (_config->GetConnection(GetSelfNum(), outs[i]->GetType(), outs[i]->GetNumber(), lPat, lType, lNum) != -1)
		{
			...
			if (outs[i]->GetType() == ioDigitalOut) // Сначала инициализируем метки
			{
				...
				for(auto l = 0; l < 1024; l++) // Метки с Id
					if (static_cast<ChArincOut*>(outs[i])->IsLabelExist(l | 0x400))
					{
						auto word = new ArincWordOut(_driversIO->GetArincOut(lNum), l);
						_driversIO->AddArincWord(word);
						static_cast<ChArincOut*>(outs[i])->GetLabel(l | 0x400)->AssignChannel(word);
						static_cast<ChArincOut*>(outs[i])->GetLabel(l | 0x400)->InitRegisters();
						outs.push_back(static_cast<ChArincOut*>(outs[i])->GetLabel(l | 0x400));
					}
			}
